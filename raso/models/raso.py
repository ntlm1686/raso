 ...
 * which is based on Recognize Anything Model (RAM), Written by Xinyu Huang.
'''
import json
import warnings
...
class TemporalAttention(nn.Module):
    ...
    def forward(self, x):
        B, T, N, D = x.shape
        x = x.permute(0, 2, 1, 3).reshape(B * N, T, D)  # shape: (B*N, T, D)
        attn_output, _ = self.attention(x, x, x)  # shape: (B*N, T, D)
        attn_output = attn_output.mean(dim=1)  # shape: (B*N, D)
        attn_output = attn_output.view(B, N, D)
        return attn_output

class RASO(nn.Module):
    def __init__(self,
                 ...
                 threshold=0.68,
                 delete_tag_index=[],
                 tag_list=f'{CONFIG_PATH}/labels.txt',
                 stage='eval',
                 frame_rate=10):
        ...
        self.video_fusion = TemporalAttention(512, 8)
        self.video_proj = nn.Linear(vision_width, 512)
        ...
        ram_class_threshold_path = f'{CONFIG_PATH}/data/sram_tag_list_threshold.txt'
    ...
    def forward(self, image, caption, image_tag, parse_tag, clip_feature, tag_only=False, video=None):
        """
        ...
        if video is not None:
            ...
            image_embeds = self.video_proj(self.visual_encoder(video))
            image_embeds = image_embeds.view(B, T, 145, 512)
            image_embeds = self.video_fusion(image_embeds)
        else:
        ...
        if tag_only:
            if video is None:
                return None, loss_tag, loss_dis
            else:
                return None, loss_tag, None
    ...
    def generate_tag(self,
        ...
        targets = torch.where(
            torch.sigmoid(logits) > self.class_threshold.to(image_embeds.device),
            torch.tensor(1.0).to(image_embeds.device),
            torch.zeros(self.num_class).to(image_embeds.device))
    ...
    def generate_tag_pooling(self,
        ...
        if video is not None:
            ...
            B, T, C, H, W = video.shape
            video = video.view(B*T, C, H, W)
            image_embeds = self.image_proj(self.visual_encoder(video))
            image_embeds = image_embeds.view(B, T, 145, 512)
